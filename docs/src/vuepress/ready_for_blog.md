...
깃허브 레포지토리에 들어가면, 오른쪽 위에 초록색 'Clone' 버튼이 있어요.  
...
Node 하나만 설치하세요. Node가 설치해준다고 하는 다른 사이드는 모두 없어도 돼요.
:::
...
$ npm -version
```
했을 때, 버젼이 출력(x.xx.x)가 출력되면 정상이에요.  
...
## 준비물 마지막: 뷰프레스
이제 뷰프레스를 여러분 블로그 폴더(레포지토리 클론)에 설치만 해 둡시다.  
:::details 쉘을 끈 적이 있다면 폴더 위치로 이동하세요.
```shell
$ cd c:/users/<username>/documents/<directoryName>
```
:::
